...
describe("SignupForm", () => {
  ...
  test.only("displays error for empty required fields", async () => {
  ...
  test("validates ID field", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("최소 5자 이상 입력해주세요.")
      ).toBeInTheDocument();
    });
    ...
    await waitFor(() => {
      expect(
        screen.getByText("최대 15자 이하로 입력해주세요.")
      ).toBeInTheDocument();
    });
  });
  ...
  test("validates email field", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("이메일 형식에 맞게 입력해주세요.")
      ).toBeInTheDocument();
    });
  });
  ...
  test("validates password field", async () => {
    ...
    fireEvent.change(passwordInput, {
      target: { value: "verylongpasswordover20chars" },
    });
    fireEvent.click(screen.getByRole("button", { name: /submit/i }));

    await waitFor(() => {
      expect(
        screen.getByText("최대 20자 이하로 입력해주세요.")
      ).toBeInTheDocument();
    });
    ...
    await waitFor(() => {
      expect(
        screen.getByText("영문과 숫자만 입력해주세요.")
      ).toBeInTheDocument();
    });
  });
